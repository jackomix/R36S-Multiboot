...
function check_deps {
    ...
    for cmd in aria2c whiptail jq parted mkfs.exfat kpartx megadl; do
    ...
    if [[ $DEPS_MISSING -eq 1 ]]; then
        ...
        if command -v apt-get >/dev/null 2>&1; then
            sudo apt-get update
            sudo apt-get install -y aria2 whiptail jq parted p7zip-full exfatprogs kpartx megatools
        else
